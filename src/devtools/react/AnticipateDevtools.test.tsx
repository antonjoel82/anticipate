...
import { AnticipateDevtools } from './AnticipateDevtools.js'
...
describe('AnticipateDevtools', () => {
  it('renders closed by default', () => {
    const engine = new TrajectoryEngine()
    const profiler = new AnticipateProfiler(engine)
    render(<AnticipateDevtools profiler={profiler} />)
    expect(screen.getByRole('button', { name: /anticipate/i })).toBeDefined()
  ...
  it('opens panel on toggle click', () => {
    ...
    const profiler = new AnticipateProfiler(engine)
    render(<AnticipateDevtools profiler={profiler} />)
    fireEvent.click(screen.getByRole('button', { name: /anticipate/i }))
  ...
  it('renders with initialIsOpen=true', () => {
    const engine = new TrajectoryEngine()
    const profiler = new AnticipateProfiler(engine)
    render(<AnticipateDevtools profiler={profiler} initialIsOpen={true} />)
    expect(screen.getByText('Predictions')).toBeDefined()
    profiler.destroy()
  })
